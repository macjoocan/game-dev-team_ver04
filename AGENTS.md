...
```
`sprite-normalize`·`atlas-pack`은 **만드는** 도구고 이건 **판정하는** 도구다.
- **미달**: 캔버스 크기 섞임 · **정체성 드리프트**(프레임 간 색 분포 — 이 축의 1번 위험) · 아틀라스 점유율
- **경고**: 떨림 · baseline 산포 · 알파 품질(고아 섬·구멍·프린지) · 축소 판독성.
  동작 종류에 따라 정상이다 — 점프는 baseline이 움직이고 던진 무기는 덩어리가 떨어져 있다.
- 떨림은 무게중심 **2차 차분**으로 본다(1차=속도는 걷기에서도 크다). 절대 px가 아니라
...
- 이 레포(플러그인 개발) 자체를 고쳤으면 `node scripts/validate-plugin.mjs`,
  ...
  CI에서는 죽는 부류가 있다(`; code=$?`, REVIEW.md B-13).
