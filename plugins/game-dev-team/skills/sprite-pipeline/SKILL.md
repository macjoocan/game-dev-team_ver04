...
```bash
node skills/sprite-pipeline/scripts/sprite-qa.mjs <프레임폴더> [--atlas atlas.json] [--aerial]
node skills/sprite-pipeline/scripts/sprite-qa.mjs <시트.png> --frames 8
```

위 도구들은 **만드는** 도구다. 이건 **판정하는** 도구다 — `sprite-normalize`는 규격을 통일하고
`atlas-pack`은 점유율을 출력하지만, 둘 다 "이게 괜찮은가"를 말하지 않는다.

| 판정 | 무엇 | 등급 |
|---|---|---|
| **캔버스 정합** | 프레임 크기가 섞여 있으면 엔진에서 정렬이 깨진다 | 미달 |
| **정체성 드리프트** | 프레임 간 색 분포(Lab 히스토그램) 유사도. **이 스킬의 1번 위험** | 미달 |
| 떨림 | 무게중심 **2차 차분** — 격렬함(최소변 대비) 또는 이상치 배수 | 경고 |
| baseline 정렬 | 발 위치 산포. 공중 동작은 `--aerial` | 경고 |
| 알파 품질 | 고아 섬 · 구멍 · 프린지 | 경고 |
| 축소 판독성 | 50% 축소 실루엣 대비 | 경고 |
| 아틀라스 점유율 | `--atlas`를 주면 낭비되는 텍스처 메모리 | 미달 |

**떨림 판정이 왜 2차 차분인가**: 1차 차분(속도)이 크다고 떨림이 아니다 — 걷기·돌진은 원래
움직인다. **프레임마다 가속이 튀는 게 떨림**이다. 그리고 절대 px로 재면 안 된다:
진폭 3px 걷기 사이클도 2차 차분이 1.9px 나온다(정상). 그래서 **스프라이트 최소변 대비 비율**과
**중위값 대비 이상치 배수**를 같이 본다 — 앞은 전체가 격렬한 경우, 뒤는 한 프레임만 튀는 경우.

- **경고는 게이트를 막지 않는다.** 동작 종류에 따라 정상이다 — 점프는 baseline이 움직이고,
  잔상 이펙트는 반투명 비율이 원래 높고, 던진 무기는 떨어진 덩어리가 의도적이다.
- **공격/피격/대기 프레임의 타이밍은 여기서 판정하지 않는다** → `polish`의 `feel-audit` 소관.
