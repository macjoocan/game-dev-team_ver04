...
### 주의 — 색 분포만으로는 **형태 붕괴**를 못 잡는다 (실측 2026-09-08)

`sprite-qa` 의 정체성 드리프트는 프레임 간 **색 히스토그램** 거리다. 영상 모델(WAN)로 뽑은 시퀀스에서
캐릭터가 파편으로 흩어져 알아볼 수 없게 됐는데도 **충족**이 나왔다 — 색 구성이 그대로였기 때문이다.
색은 같고 실루엣만 무너지는 경우가 여기 해당한다. **생성 시퀀스는 판정 통과 후에도 사람이 한 번 본다.**
