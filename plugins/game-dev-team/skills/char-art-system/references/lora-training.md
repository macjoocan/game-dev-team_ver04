...
> **플랫폼**: 절차는 Windows + CUDA 기준이고 경로(`D:\sd-scripts` 등)는 예시다. Linux + NVIDIA는
> `SD_SCRIPTS_DIR`만 맞추면 그대로 돈다. **CUDA가 없는 환경(Apple Silicon 등)에서는 학습이 안 되므로
> 판정은 미달이 아니라 `측정 불가`** 다 — 되돌릴 곳은 기획이 아니라 환경 세팅이다.
