...
```bash
node skills/ui-art-system/scripts/pseudo-loc.mjs <strings.json> --spec ui-spec.json [--expand 0.35]
node skills/ui-art-system/scripts/pseudo-loc.mjs <strings.json> --write pseudo.json
```

텍스트 오버플로는 게임 로컬라이제이션 버그 1위이고, **의사 로케일**(문자열을 목표 팽창률만큼
늘리고 액센트로 치환)이 그 종류의 약 80%를 번역이 오기 전에 잡는다. 액센트 치환은 부수 효과로
**하드코딩된 문자열**을 즉시 드러낸다 — 화면에 평범한 텍스트가 남아 있으면 코드에 박힌 것이다.

**한국어 프로젝트는 위험 방향이 반대다.** 한국어·중국어·일본어는 같은 뜻을 더 적은 글자로
표현하는 **짧은** 언어고, 독일어·러시아어는 20~35% 팽창한다. 즉 **한국어로 폭을 맞추면
영어에서 이미 빠듯하고 독일어에서 터진다.** 리포트가 "영문이 원문보다 15% 이상 넓은 문자열"을
따로 뽑아주니 거기부터 본다.

- `ui-spec.json`을 함께 주면 **9-slice 보더 안쪽 폭**과 대조해 넘침을 계산 판정한다.
  안 주면 길이 배수만 나오고 판정은 `측정 불가`다.
- `{0}`·`%s` 같은 치환 토큰은 보존한다(깨지면 런타임 오류다). 손상되면 리포트가 잡는다.
- 폭은 문자 부류별 상대 전각(em) 근사다 — 정확한 픽셀이 아니라 **어느 문자열이 위험한가**를
  가린다. 경계선 문자열은 실기기에서 확인해라.
- 줄바꿈·자동 축소·말줄임 설정은 이 도구가 모른다. 판정 후 실제 화면을 `visual-qa`로 본다.
- 세로 넘침(줄 수 증가)과 RTL 미러링은 범위 밖이다.
...
## 9-slice 보더는 도구가 계산한다 (실측 2026-09-17)

`ui-kit-gen` 이 manifest 에 `nineSlice` 를 **자동으로 낸다.** 예전엔 스펙이 적어준 값을
그대로 흘려보내기만 해서 거의 항상 `null` 이 나갔는데, 콘솔은 "9-slice 보더가 들어 있다"고
말하고 있었다 — **도구가 안 한 일을 했다고 말하는 상태**였다.

보더가 없으면 늘릴 때 **모서리 반경과 선 두께가 같이 늘어난다.** 96x32 버튼을 240px 로
늘려보면 눈에 바로 보인다. 계산식은 도구가 그릴 때 이미 아는 값이다:

```
보더 = 올림(여백 + 모서리반경 + 선두께 + 그림자오프셋)
```

양쪽 보더 합이 변보다 크면 9-slice 가 성립하지 않으므로 절반 미만으로 조인다.
스펙에 `nineSlice` 를 직접 주면 그쪽이 우선한다.

실측 검증: 버튼·패널·바를 가로 2.3배·세로 2배로 늘려 **8건 전부 모서리 차이 0.000**.

> **검증할 때 `alpha_composite` 를 쓰지 마라.** 반투명 픽셀에서 반올림 오차가 생겨
> 멀쩡한 보더가 깨진 것처럼 나온다(실측: 그림자 블러가 모서리의 35% 인 패널에서 1.26).
> 1:1 복사 구간은 `paste` 로 비교해야 한다.

## 상태 변형은 픽셀 단위로 정합해야 한다

normal/pressed/disabled 의 **알파 실루엣이 같아야** 버튼이 눌릴 때 안 흔들린다.
실측에서 `shadow.offset` 만 다르게 줬더니 실루엣이 2.21% 어긋났다 — 도구 문제가 아니라
스펙 문제다. 상태 간에는 **색·하이라이트만 바꾸고 형태 값(pad·radius·stroke·shadow)은 고정**한다.
