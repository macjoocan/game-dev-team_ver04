...
- `char-art-system으로 채택된 시안 15장으로 캐릭터 LoRA 학습시켜줘` (정체성 고정 — 학습기 없으면 측정 불가)
...
## 7. 게이트 체크(사람용 요약)
- **게이트 모드**: `lean`(핵심 게이트만: 코어 확정·게임성 검증·머지, **기본**) / `full`(모든 게이트 — 정식·출시 준비 시 명시 선택) / `solo`(게이트 없음, 잼/실험용). 세팅 시 또는 진행 중 언제든 변경 가능 — AGENTS.md에 기록된다.
